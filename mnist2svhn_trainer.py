...
flags = tf.app.flags
flags.DEFINE_float('lamda', 0.01, "value of lamda")
flags.DEFINE_float('learning_rate', 0.01, "value of learnin rate")
FLAGS = flags.FLAGS
N_CLASS = 10
# input data path!
path_svhn_train = ''
...
class SVHNModel(object):
    ...
    def _build_model(self):
        ...
        with tf.variable_scope('feature_extractor'):
            ...
            h_fc1_drop = tf.reshape(h_conv2, [-1, 8192])
        ...
        with tf.variable_scope('label_predictor_2'):
            ...
            h_fc0_2 = tf.nn.relu(tf.matmul(self.feature, W_fc0_2) + b_fc0_2)
            ...
            logits2 = tf.nn.l2_normalize(logits2, 1)
        ...
        for t in xrange(30):
            ...
            if t == 0:
                gen_source_only_batch = batch_generator(
                    [data_s_im, data_s_label], batch_size / 2)
                gen_target_only_batch = batch_generator(
                    [data_t_im, data_t_label], batch_size / 2)
            ...
            for i in range(num_steps):
                ...
                if t == 0:
                    ...
                    X1, _ = gen_target_only_batch.next()
                    X = np.vstack([X0, X1])
                    y = np.vstack([y0, y0])
                    ...
                    _, _, batch_loss, w_diff, ploss, p_l1, p_l2, p_acc1, p_acc2 = \
                        sess.run([target_train_op, regular_train_op1, total_loss, weight_diff, total_loss, pred_loss1,
                                 ...
                                 feed_dict={model.X: X, model.y: y,
                                            model.train: True, learning_rate: lr, model.keep_prob: dropout})
                ...
                if t >= 1:
                    ...
                    Xt, _ = gen_target_only_batch.next()
                    X = np.vstack([X0, Xt])
                    y = np.vstack([y0, y0])
                    _, batch_loss, w_diff, ploss, p_l1, p_l2, p_acc1, p_acc2 = \
                        sess.run([regular_train_op1, total_loss, weight_diff, total_loss, pred_loss1, pred_loss2,
                                 ...
                                 feed_dict={model.X: X, model.y: y, model.train: True, learning_rate: lr,
            ...
            if t == 0:
                cand = data_t_im[perm, :]
                rate = max(int((t + 1) / 20.0 * pred1_stack.shape[0]), 5000)
                new_data, new_label = judge_func(cand,
                                                 pred1_stack[:rate, :],
                                                 pred2_stack[:rate, :],
                                                 num_class=N_CLASS, upper=0.95)
            if t != 0:
                cand = data_t_im[perm, :]
